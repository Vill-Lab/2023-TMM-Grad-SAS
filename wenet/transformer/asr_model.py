...
from wenet.transformer.encoder import Identity
...
class ASRModel(torch.nn.Module):
    ...
    def forward(
        ...
        # 1. Encoder
        encoder_out, encoder_mask = self.encoder(speech, speech_lengths, drop_masks)
        ...
        if self.ctc_weight != 0.0:
            if score:
                loss_ctc, score_mat = self.ctc(encoder_out, encoder_out_lens, text,
                                text_lengths, score=True)
            else:
        ...
        if score:
            return loss, loss_att, loss_ctc, score_mat
        else:
    ...
    def ctc_greedy_search(
        ...
        return hyps#, ctc_probs, predIdx

    def ctc_greedy_search_with_prob(
        self,
        speech: torch.Tensor,
        speech_lengths: torch.Tensor,
        decoding_chunk_size: int = -1,
    ...
    def ctc_greedy_search_with_removal(
        self,
        xs, pos_emb, masks,
        decoding_chunk_size: int = -1,
        num_decoding_left_chunks: int = -1,
        ...
        # batch_size = speech.shape[0]
        # Let's assume B = batch_size
        # xs, pos_emb, masks = self.encoder.prepare(speech, speech_lengths, remove_i)
        batch_size = xs.shape[0]
        encoder_out, encoder_mask = self.encoder.forward_after_prepare(xs, pos_emb, masks, decoding_chunk_size,
            num_decoding_left_chunks)
        # encoder_out, encoder_mask = self._forward_encoder(
        #     speech, speech_lengths, decoding_chunk_size,
        #     num_decoding_left_chunks,
