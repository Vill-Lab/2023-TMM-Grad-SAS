...
class AVSRModel(torch.nn.Module):
    ...
    def forward(
        ...
        # 1. Encoder
        a_encoder_out, a_encoder_mask = self.audio_encoder(speech, speech_lengths)
        v_encoder_out, v_encoder_mask = self.video_encoder(visual, speech_lengths)
        # encoder_out.register_hook(self.get_grad)
        encoder_out = self.fusion_layer(a_encoder_out, a_encoder_mask, v_encoder_out, v_encoder_mask)
        encoder_out_lens = a_encoder_mask.squeeze(1).sum(1)
...
def init_avsr_model(configs):
    ...
    audio_input_dim = configs['input_dim']
    vocab_size = configs['output_dim']

    audio_encoder_type = configs.get('audio_encoder', 'conformer')
    video_encoder_type = configs.get('video_encoder', 'conformer')
    decoder_type = configs.get('decoder', 'bitransformer')

    if audio_encoder_type == 'conformer':
        audio_encoder = ConformerEncoder(audio_input_dim,
                                   global_cmvn=global_cmvn,
                                   **configs['audio_encoder_conf'])
    else:
        audio_encoder = TransformerEncoder(audio_input_dim,
                                     global_cmvn=global_cmvn,
                                     **configs['audio_encoder_conf'])
    
    if video_encoder_type == 'conformer':
        video_encoder = ConformerEncoder(video_input_dim,
                                   global_cmvn=global_cmvn,
                                   **configs['video_encoder_conf'])
    else:
        video_encoder = TransformerEncoder(video_input_dim,
                                     global_cmvn=global_cmvn,
                                     **configs['video_encoder_conf'])    
    if decoder_type == 'transformer':
        decoder = TransformerDecoder(vocab_size, audio_encoder.output_size(),
                                     **configs['decoder_conf'])
    else:
        assert 0.0 < configs['model_conf']['reverse_weight'] < 1.0
        assert configs['decoder_conf']['r_num_blocks'] > 0
        decoder = BiTransformerDecoder(vocab_size, audio_encoder.output_size(),
                                       **configs['decoder_conf'])
    ctc = CTC(vocab_size, audio_encoder.output_size())
    model = AVSRModel(
