...
# from wenet.transformer.attention import RelPositionMultiHeadedAttention_transposed
...
from wenet.transformer.encoder_layer import IdentityEncoderLayer
...
class BaseEncoder(torch.nn.Module):
    def __init__(
        ...
        self.lime_layer = None
        self.lime_layer_grad = None
    ...
    def forward_after_prepare(self, xs, pos_emb, masks,
    ...
    def lime_layer_hook(self, grad):
        self.lime_layer_grad = grad
    ...
    def forward(
        ...
        chunk_masks = add_optional_chunk_mask(xs, masks,
                                              ...
                                              self.static_chunk_size,
                                              num_decoding_left_chunks)
        self.lime_layer = torch.ones([1, xs.shape[1], 1]).requires_grad_(True)
        self.lime_layer.register_hook(self.lime_layer_hook)
        xs = self.lime_layer * xs
        for layer in self.encoders:
            xs, chunk_masks, _ = layer(xs, chunk_masks, pos_emb, mask_pad)
        if self.normalize_before:
            xs = self.after_norm(xs)
...
class Identity(TransformerEncoder):
    """Transformer encoder module."""
    def __init__(
        self,
        input_size: int,
