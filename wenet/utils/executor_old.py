        ...
        for batch_idx, batch in enumerate(data_loader):
            ...
            with context():
                ...
                # https://pytorch.org/docs/stable/notes/amp_examples.html
                    ...
                    drop_masks = drop_masks[v > v.max() * 0.8]
                    # optimizer.zero_grad()
                    # autocast context
                    # The more details about amp can be found in
                    # https://pytorch.org/docs/stable/notes/amp_examples.html
                ...
                elif args['train_method'] == 'drop_2mask':
                # autocast context
                # The more details about amp can be found in
                # https://pytorch.org/docs/stable/notes/amp_examples.html
                    if use_amp:
                    ...
                    with torch.cuda.amp.autocast(scaler is not None):
                        loss, loss_att, loss_ctc = model(feats, feats_lengths,
                                                            target, target_lengths, drop_masks)
                        loss = loss / accum_grad
                    # for the 2nd round of drop_mask
                    if use_amp:
                        scaler.scale(loss).backward()
                    else:
                        loss.backward()
                    ...
                    # drop_masks = drop_masks_v[drop_masks_v > drop_masks_v.max() * 0.8]
                    # v, drop_masks = torch.topk(drop_masks_v, int(len(drop_masks_v) * args['att_dropout_rate']))
                    v2, drop_masks2 = torch.topk(drop_masks_v, int(len(drop_masks_v)))
                    drop_masks2 = drop_masks2[(v2 > v2.max() * 0.8) or (v > v.max() * 0.8)]
                    # optimizer.zero_grad()
                    # autocast context
                    # The more details about amp can be found in
                    # https://pytorch.org/docs/stable/notes/amp_examples.html
                    with torch.cuda.amp.autocast(scaler is not None):
                        loss, loss_att, loss_ctc = model(feats, feats_lengths,
                                                            target, target_lengths, drop_masks2)
                        loss = loss / accum_grad
                    if use_amp:
                        scaler.scale(loss).backward()
                    else:
                ...
                elif args['train_method'] == 'diags':
                    if use_amp:
                        scaler.scale(loss).backward(retain_graph=True)
                    else:
                        loss.backward(retain_graph=True)
                    diagloss = (args['diag_ratio'] - get_diags(model, diags=[-2,-1,0,1,2])).clamp(0)
                    optimizer.zero_grad()
                    if use_amp:
                        scaler.scale((loss + diagloss)).backward()
                    else:
                        (loss + diagloss).backward()
